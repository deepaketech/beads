jQuery.noConflict();jQuery(function($){SearchAutocomplete=Backbone.View.extend({url:null,result:null,suggest:null,query:null,delay:100,store:1,queryMinChars:3,shown:false,events:{'keydown .UI-SEARCH':'onKeydown','keyup .UI-SEARCH':'onKeyup','blur .UI-SEARCH':'onBlur','focus .UI-SEARCH':'onFocus','change .UI-CATEGORY':'onChangeCategory'},initialize:function()
{var self=this;_.bindAll(self);},init:function(el)
{var self=this;self.setElement(el);self.url=self.$el.attr('data-url');self.delay=self.$el.attr('data-delay');self.queryMinChars=self.$el.attr('data-minchars');self.store=self.$el.attr('data-store');self.$listenField=$('.UI-SEARCH',self.$el);self.$suggestField=$('.UI-SEARCH-SUGGEST',self.$el);self.$placeholder=$('.UI-PLACEHOLDER',self.$el);self.$loader=$('.UI-LOADER',self.$el);self.$category=$('.UI-CATEGORY',self.$el);self.$categoryText=$('.UI-CATEGORY-TEXT',self.$el);self.result='';self.query=self.$listenField.val();self.baseWidth=$('.UI-NAV-INPUT').width();self.onChangeCategory();},onChangeCategory:function()
{var self=this;var text=$('option:selected',self.$category).text();if(self.$categoryText.length){self.$categoryText.html(text);var width=self.$categoryText.width();if(width<0){width=20;}
...
{var ts=new Date().getTime();var self=this;query=query.replace(/<\/?[^>]+>/gi,'');var cat='';if(self.$category.val()!==undefined){cat='&'+self.$category.attr('name')+'='+self.$category.val();}
...
console.log(new Date().getTime()-ts);}});},show:function(html)
...
self.$placeholder.show();self.$el.addClass('active');self.shown=true;$('li',self.$placeholder).each(function(key,el){var $el=$(el);$el.on('mouseover',self.mouseover);$el.on('mouseout',self.mouseout);$el.on('click',self.click);});return this;},hide:function()
{var self=this;self.$placeholder.hide();self.$el.removeClass('active');self.shown=false;return this;},highlighter:function(item)
{var self=this;$('.highlight',item).each(function(key,el){var $el=$(el);var arQuery=self.$listenField.val().split(' ');var html=$el.text();arQuery.each(function(word){if($.trim(word)){word=word.replace(/[\-\[\]{}()*+?.,\\\^$|#\s]/g,'\\$&');html=html.replace(new RegExp('('+word+'(?![^<>]*>))','ig'),function($1,match){return'<strong>'+match+'</strong>';});}});$el.html(html);});}});$(function(){_.each($('.UI-SEARCHAUTOCOMPLETE'),function(el){var autocomplete=new SearchAutocomplete();autocomplete.init(el);});});});