...
function cancel(){
	...
	//jQuery("#title").removeClass('disabletitle');
	...
	jQuery("#group_id").val(group_id);
		...
		new Ajax.Request(url, {
			...
			if (transport.responseText){
				//$('parent_id').innerHTML=transport.responseText;
				jQuery('#parent_id').empty();
				jQuery('#parent_id').append(transport.responseText);
				var parent_item = document.getElementById('parent_id').value;
				//show_hide_layout_align(parent_item);
...
function show_hide_layout_align(parent){
	
	/*if((parseInt(parent) == "0") || (parseInt(parent) == "")){
	jQuery("#subcolumnlayout").removeAttr('disabled');
	jQuery("#subcolumn").show();
	jQuery("#text_align").removeAttr('disabled');
	jQuery("#text_align_content").show();
	}else {
	jQuery("#subcolumnlayout").attr('disabled',true);
	jQuery("#subcolumn").hide();
	jQuery("#text_align").attr('disabled',true);
	jQuery("#text_align_content").hide();
	}*/
...
function showhideelement(menu_type){
	if(parseInt(menu_type) == "1"){
		...
		//jQuery("#use_category_title").attr('disabled',true);
		...
		//jQuery("#use_category_title").removeAttr('disabled');
...
jQuery("#deletedialog").css("display", "block");
 ...
 var deleteoption = document.getElementById('deleteparent').value;
  ...
  var url = '';
	var url = delete_url+"deleteoption/"+deleteoption;
	console.log("Delete Parent & child"+url);
}
	...
	if(group_id != ''){
		...
		new Ajax.Request(url, {
			...
			if (transport.responseText){
				...
				//jQuery('parent_id').Html=transport.responseText;
				jQuery('#parent_id').empty();
				jQuery('#parent_id').append(transport.responseText);
				//$('parent_id').innerHTML=transport.responseText;
...
function change_title(){
	 /*var remember = document.getElementById('use_category_title');
    if (remember.checked){
	document.getElementById('use_category_title').value = "1";
	document.getElementById("title").readOnly = false;
    jQuery("#title").removeClass('disabletitle'); 
    }else {
	document.getElementById('use_category_title').value = "0";
	document.getElementById("title").readOnly = true;
    document.getElementById("title").className += " disabletitle";
    }*/
	}
