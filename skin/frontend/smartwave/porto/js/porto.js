function setNewsletterCookie(){ jQuery.cookie('newsletter_popup','dontshowitagain'); }
jQuery.fn.extend({ scrollToMe: function(){ if(jQuery(this).length){ var top = jQuery(this).offset().top - 100; jQuery('html,body').animate({scrollTop: top}, 300); } },
    scrollToJustMe: function(){ if(jQuery(this).length){ var top = jQuery(this).offset().top; jQuery('html,body').animate({scrollTop: top}, 300); } } });
function portoAlert(msg){ jQuery('<div class="note-msg container alert" style="display:none;position:fixed;top:30px;margin-left:-30px; z-index:9999;">'+msg+'</div>').appendTo("div.main"); jQuery(".alert").fadeIn(500);
...
jQuery(function($){
    $('div.product-view p.no-rating a, div.product-view .rating-links a').click(function(){
        if($('#tab_review_tabbed').hasClass("panel-collapse")){
            ...
            setTimeout(function(){ $('#product_tab_accordion').scrollToJustMe(); },200); } else {
            $('.product-tabs ul li').removeClass('active');
            $('#tab_review_tabbed').addClass('active');
            $('.product-tabs .tab-content').hide();
            $('#tab_review_tabbed_contents').show();
            $('#tab_review_tabbed').scrollToMe(); } return false; });    
    $(".word-rotate").each(function() {
        var $this = $(this),itemsWrapper = $(this).find(".word-rotate-items"),items = itemsWrapper.find("> span"),firstItem = items.eq(0),firstItemClone = firstItem.clone(),itemHeight = 0,currentItem = 1,currentTop = 0;
        itemHeight = firstItem.height();
        itemsWrapper.append(firstItemClone);
        $this.height(itemHeight).addClass("active");
        setInterval(function() { currentTop = (currentItem * itemHeight);
    ...
    $(window).stellar({ responsive: true,scrollProperty: 'scroll',parallaxElements: false,horizontalScrolling: false,horizontalOffset: 0,verticalOffset: 0 });
/********** Fullscreen Slider ************/
...
/************** Header - Search icon, Links icon click event ***************/
    ...
    $(".move-action .item .details-area .actions").each(function(){ $(this).parent().parent().children(".product-image-area").append($(this)); });
});
jQuery(function($){
    var breadcrumb_pos_top = 0;
    $(window).scroll(function(){ if(!$("body").hasClass("cms-index-index")){ var side_header_height = $(".header-container.type12").innerHeight(); var window_height = $(window).height();
            if(side_header_height-window_height<$(window).scrollTop()){ if(!$(".header-container.type12").hasClass("fixed-bottom"))
        ...
        if($("body.side-header .top-container > .breadcrumbs").length){
            ...
            }else{ if($(window).scrollTop()<=1){ $("body.side-header .top-container > .breadcrumbs").removeClass("fixed-position"); }
            }
        }
    });
});