 ...
 */
class Amasty_Customerattr_Model_Rewrite_Customer
    extends Mage_Customer_Model_Customer
{
    ...
    public function getAttributes()
    {
        ...
        foreach ($collection as $attribute) {
            if ('customer_activated' != $attribute->getAttributeCode()
                && 'unlock_customer' != $attribute->getAttributeCode()
            ) {
                // filter attributes by store on the edit customer page in the backend
                if ('customer' == Mage::app()->getRequest()->getControllerName()
                    && 'edit' == Mage::app()->getRequest()->getActionName()
                ) {
                    $applicableStoreIds = explode(
                        ',', $attribute->getStoreIds()
                    );
                    // 0 means allowed on all store views
                    if (!in_array(0, $applicableStoreIds)) {
                        if (!in_array(
                                Mage::registry('current_customer')->getStoreId(
                                ), $applicableStoreIds
                            )
                            && 0 != Mage::registry('current_customer')
                                ->getStoreId()
                        ) {
                            continue;
                        }
    ...
    public function loadByEmail($customerEmail)
    {
        if ('forgotpasswordpost' == Mage::app()->getRequest()->getActionName()
            ...
            if (!Mage::getStoreConfig('amcustomerattr/forgot/disable_email')
                || !Mage::getStoreConfig('amcustomerattr/forgot/forgot_field')
            ) {
            ...
            if (Mage::getStoreConfig('amcustomerattr/forgot/forgot_field')) {
                ...
                $attribute = Mage::getModel('customer/attribute')->load(
                    Mage::getStoreConfig('amcustomerattr/forgot/forgot_field'),
                    'attribute_code'
                );
                if ($attribute->getId()) {
                    $this->_getResource()->loadByAttribute(
                        $this, $customerEmail, $attribute
                    );
                }
            }
        } else { // Login
            ...
            if (!Mage::getStoreConfig('amcustomerattr/login/disable_email')
                || !Mage::getStoreConfig('amcustomerattr/login/login_field')
            ) {
                parent::loadByEmail($customerEmail);
                if ($this->getId()) {
                    // customer found by e-mail, no need to load by attribute
    ...
    public function custom($attributeCode)
    {
        if ('group_name' == $attributeCode) {
            ...
            if ($this->getGroupId()) {
                $group = Mage::getModel('customer/group')->load(
                    $this->getGroupId()
                );
    ...
    protected function _afterSave()
    {
        if ($this->_fileAttributes) {
            $deleteFiles = Mage::app()->getRequest()->getPost(
                'amcustomerattr_delete'
            );
            foreach ($this->_fileAttributes as $id => $attributeCode) {
                if (isset($_FILES['amcustomerattr']['error'][$attributeCode])
                    && UPLOAD_ERR_OK
                    == $_FILES['amcustomerattr']['error'][$attributeCode]
                ) {
                    try {
                        $fileName
                    ...
                    if ($this->_savedFileNames[$attributeCode]) {
                        Mage::helper('amcustomerattr')->deleteFile(
                            $this->_savedFileNames[$attributeCode]
                        );
                    }
                }
                if ($deleteFiles && $deleteFiles[$attributeCode]
                    && ($this->_savedFileNames[$attributeCode]
                        === $deleteFiles[$attributeCode])
                ) {
                    Mage::helper('amcustomerattr')->deleteFile(
                        $deleteFiles[$attributeCode]
                    );
                }
                unset($_FILES['amcustomerattr']['name'][$attributeCode]);
                unset($_FILES['amcustomerattr']['type'][$attributeCode]);
                unset($_FILES['amcustomerattr']['tmp_name'][$attributeCode]);
                unset($_FILES['amcustomerattr']['error'][$attributeCode]);
                unset($_FILES['amcustomerattr']['size'][$attributeCode]);
            }
        }

        if (!$this->getOrigId()
            && !Mage::registry(
                'amcustomerattr_customer_registry_dispatched'
            )
        ) {
            Mage::register('amcustomerattr_customer_registry_dispatched', true);
            Mage::dispatchEvent(
                'amcustomerattr_customer_registry', array('model' => $this)
            );
        }
        ...
        if ($origData === null) { // new user registration will be
            $notified = Mage::registry('AdminNotifiedAboutNewUser');
            if (Mage::getStoreConfig(
                    'amcustomerattr/activation/notify_admin',
                    $this->getStoreId()
                )
                && !$notified
            ) {
                ...
                $template = Mage::getStoreConfig(
                    'amcustomerattr/activation/email_template_admin',
                    $this->getStoreId()
                );
                $template = $template ? $template
                    : 'amasty_amcustomerattr_activation_email_template_admin';

                $emails = Mage::getStoreConfig(
                    'amcustomerattr/activation/admin_email', $this->getStoreId()
                );
                foreach (explode(',', $emails) as $email) {
                    ...
                    $tpl->setDesignConfig(
                        ...
                        ->sendTransactional(
                            $template,
                            Mage::getStoreConfig(
                                'amcustomerattr/activation/admin_sender',
                                $this->getStoreId()
                            ),
                            $email,
                            'Admin notification',
                            array(
                                ...
                                'created_at'    => date(
                                    'Y-m-d', $this->getCreatedAtTimestamp()
                                ),
                                'customer'      => $this,
                            ),
                            $this->getStoreId()
         ...
         */
        $paymentPost = Mage::app()->getRequest()->getPost(
            'payment'
        ); //exclude checkout pages
        if (Mage::getStoreConfig(
                'amcustomerattr/activation/notify_customer', $this->getStoreId()
            )
            && !$paymentPost
            && $this->getOrigData('am_is_activated') != $this->getData(
                'am_is_activated'
            )
            && self::ACTIVATION_STATUS_ACTIVATED == $this->getData(
                'am_is_activated'
            )
        ) {
            ...
            if (!$notified) {
                ...
                $translate->setTranslateInline(false);
                $template = Mage::getStoreConfig(
                    'amcustomerattr/activation/email_template_customer',
                    $this->getStoreId()
                );
                $template = $template ? $template
                    : 'amasty_amcustomerattr_activation_email_template_customer';

                $tpl = Mage::getModel('core/email_template');
                $tpl->setDesignConfig(
                    array('area' => 'frontend', 'store' => $this->getStoreId())
                )
                    ->sendTransactional(
                        $template,
                        Mage::getStoreConfig(
                            'amcustomerattr/activation/customer_sender',
                            $this->getStoreId()
                        ),
                        $this->getEmail(),
            ...
            if (!Mage::getStoreConfig(
                    'amcustomerattr/activation/auto_activation',
                    $this->getStoreId()
                )
                || (Mage::getStoreConfig(
                        'amcustomerattr/activation/auto_activation',
                    ...
                    && Mage::getStoreConfig(
                        'amcustomerattr/activation/enabled_customer_groups',
                        $this->getStoreId()
                    )
                    && self::ACTIVATION_STATUS_ACTIVATED
                    != $this->getAmIsActivated())
            ) {
                if (!$this->isConfirmationRequired()
                ) { // We do not need logout, it is Magento's work in this case
                    $this->setIsLoggedIn(true);
                    Mage::getSingleton('customer/session')->logout();
                    $customerSession = Mage::getSingleton('customer/session');
                    $customerSession->unsetAll();
                    $customerSession->getCookie()->delete(
                        $customerSession->getSessionName()
                    );
                }
                if (!Mage::registry('CustomerNotificationAboutActivation')
                    && Mage::getStoreConfig(
                        'amcustomerattr/activation/registration_notice',
                        $this->getStoreId()
                    )
                ) {
    ...
    protected function _beforeSave()
    {
        ...
        foreach ($collection as $attribute) {
            ...
            if ('file' == $attribute->getTypeInternal()) {
                $this->_fileAttributes[$attribute->getId()]
                    = $attribute->getAttributeCode();
                $filesFields[] = $attribute->getAttributeCode();
                $filesRestrictions[$attribute->getAttributeCode()]['size']
                    = 1048576 * $attribute->getFileSize();
                $filesRestrictions[$attribute->getAttributeCode()]['type']
                    = $attribute->getFileTypes();
                $filesRestrictions[$attribute->getAttributeCode()]['dimentions']
                    = $attribute->getFileDimentions();
                $this->_savedFileNames[$attribute->getAttributeCode()]
            ...
            if ($attribute->getIsUnique()) {
                ...
                if (isset($translations[Mage::app()->getStore()->getId()])) {
                    $attributeLabel = $translations[Mage::app()->getStore()
                        ->getId()];
                } else {
        ...
        if ($nameGroupAttribute
            && ($backendPost = Mage::app()->getRequest()->getPost('account'))
            && isset($backendPost['group_id'])
        ) {
            $entityType = Mage::getSingleton('eav/config')->getEntityType(
                'customer'
            );
            $groupAttribute = Mage::getModel('customer/attribute')->loadByCode(
                $entityType, $nameGroupAttribute
            );
            $storeIds = explode(',', $groupAttribute->getData('store_ids'));
            if (in_array($this->getStoreId(), $storeIds)
                || in_array(
                    0, $storeIds
                )
            ) {
                ...
                foreach ($options as $option) {
                    $temp = Mage::getModel('eav/entity_attribute_option')->load(
                        $option['value']
                    );
                    if ($temp && $temp->getGroupId()
                        && $temp->getGroupId() == $backendPost['group_id']
                    ) {
        ...
        if ('checkout' == Mage::app()->getRequest()->getModuleName()
            ...
            $customerAttributes = Mage::getSingleton('checkout/session')
                ->getAmcustomerattr();
        }
        if (!$customerAttributes) {
            $customerAttributes = Mage::app()->getRequest()->getPost(
                'amcustomerattr'
            );
        }
        if (!$customerAttributes) {
        ...
        if ($customerAttributes) {
            ...
            if ($filesFields) {
                ...
                foreach (
                    $customerAttributes as $attributeCode => $attributeValue
                ) {
            ...
            foreach ($customerAttributes as $attributeCode => $attributeValue) {
                ...
                if (in_array($attributeCode, $castDate)) {
                    if ($attributeValue) {
                        ...
                        $attributeValue = date('Y-m-d', $temp->getTimestamp());
                    }
                }
            ...
            foreach ($filesFields as $attributeCode) {
                if (isset($_FILES['amcustomerattr']['error'][$attributeCode])
                    ...
                    if (file_exists(
                        ...
                        $attributeValue = DS . $f1 . DS . $f2 . DS . $fileName
                            . '.' . $ext;
                    }
                    $_FILES['amcustomerattr']['name'][$attributeCode]
                        = $attributeValue;
                } elseif ($deleteFiles[$attributeCode]
                    && ($this->_savedFileNames[$attributeCode]
                        === $deleteFiles[$attributeCode])
            ...
            if ($idGroupSelect) {
                ...
                if ($this->getData(
                    Mage::helper('amcustomerattr/group')->getAttribute()
                )
                ) {
                    $this->setGroupId(
                        Mage::helper('amcustomerattr/group')->getGroupId()
                    );
                }
            }
        }
        ...
        if ($checkUnique) {
            foreach ($checkUnique as $attributeCode => $attributeLabel) {
                ...
                $customerCollection->addAttributeToFilter(
                    $attributeCode,
                    array('eq' => $this->getData($attributeCode))
                );
                $mainAlias = (false !== strpos(
                        $customerCollection->getSelect()->__toString(), 'AS `e'
                    )) ? 'e' : 'main_table';
                $customerCollection->getSelect()->where(
                    $mainAlias . '.entity_id != ?', $this->getId()
                );
                ...
                if (isset($_FILES['amcustomerattr']['error'][$attributeCode])
                    ...
                    if ($filesRestrictions[$attributeCode]['size']
                        && ($filesRestrictions[$attributeCode]['size']
                            < $_FILES['amcustomerattr']['size'][$attributeCode])
                    ) {
                        $fileErrors[] = Mage::helper('amcustomerattr')->__(
                    ...
                    if ($filesRestrictions[$attributeCode]['type']) {
                        ...
                        if (!in_array(
                            $ext, explode(
                                ',', $filesRestrictions[$attributeCode]['type']
                            )
                        )
                        ) {
                            $fileErrors[] = Mage::helper('amcustomerattr')->__(
                                'File ext restriction: %s',
                                $filesRestrictions[$attributeCode]['type']
                            );
                        }
                    }
                    // check type of file
                    if (substr_count(
                            $_FILES['amcustomerattr']['type'][$attributeCode],
                            '/'
                        ) > 1
                    ) { // check double file type
                        $fileErrors[] = Mage::helper('amcustomerattr')->__(
                            'Not supported type of file: %s',
                            $_FILES['amcustomerattr']['type'][$attributeCode]
                        );
                    }
                    // check content (MIME) type of file
                    if (Mage::getStoreConfig(
                            'amcustomerattr/general/check_file_type'
                        )
                        ...
                        $fileErrors[] = Mage::helper('amcustomerattr')->__(
                            'Not supported content type of file: %s',
                            $_FILES['amcustomerattr']['type'][$attributeCode]
                        );
                    }
                    // check dimentions for image
                    if ($filesRestrictions[$attributeCode]['dimentions']
                        && (false !== strpos(
                                $_FILES['amcustomerattr']['type'][$attributeCode],
                                'image/'
                            ))
                        ...
                        $dimentions = explode(
                            '/',
                            $filesRestrictions[$attributeCode]['dimentions']
                        );
                        try {
                            $imageInfo = getimagesize(
                                $_FILES['amcustomerattr']['tmp_name'][$attributeCode]
                            );
                        } catch (Exception $error) {
                            $e = new Mage_Customer_Exception(
                                Mage::helper('amcustomerattr')->__(
                        ...
                        if (($imageInfo[0] > $dimentions[0])
                            || ($imageInfo[1] > $dimentions[1])
                        ) {
                            $fileErrors[] = Mage::helper('amcustomerattr')->__(
                                'Image size restriction: %s pixels (width/height)',
                                $filesRestrictions[$attributeCode]['dimentions']
                            );
                        }
                    }
                    ...
                    if ($fileErrors) {
                        $e = new Mage_Customer_Exception(
                            Mage::helper('amcustomerattr')->__(
                                'File error: %s', implode('; ', $fileErrors)
                            )
                        );
                        if (method_exists($e, 'setMessage')) {
                            $e->setMessage(
        ...
        $accountPost = Mage::app()->getRequest()->getPost('account');
        if ($accountPost) { //set activation status
            $this->setData(
            ...
            && Mage::getStoreConfig(
                'amcustomerattr/activation/auto_activation', $this->getStoreId()
            )
            ...
            if (Mage::getStoreConfig(
                ...
                $groups = explode(
                    ',', Mage::getStoreConfig(
                    'amcustomerattr/activation/customer_groups',
                    $this->getStoreId()
                )
                );
                if (in_array($this->getGroupId(), $groups)) {
                    $this->setData(
                        'am_is_activated', self::ACTIVATION_STATUS_ACTIVATED
                    );
                }
            } else {
                $this->setData(
    ...
    public function checkContentType($ext, $contentType)
    {
        $mimeTypes = array(
            ...
            "dwg"       => array("application/acad", "image/vnd.dwg",
                                 "image/x-dwg"),
            "ez"        => "application/andrew-inset",
            "ccad"      => "application/clariscad",
            "drw"       => "application/drafting",
            "tsp"       => "application/dsptype",
            "dxf"       => array("application/dxf", "image/vnd.dwg",
                                 "image/x-dwg"),
            ...
            "class"     => array("application/octet-stream", "application/java",
                                 "application/java-byte-code",
                                 "application/x-java-class"),
            "dms"       => "application/octet-stream",
            "exe"       => array("application/octet-stream",
                                 "application/x-msdos-program"),
            "lha"       => array("application/octet-stream", "application/lha",
                                 "application/x-lha"),
            ...
            "xlc"       => array("application/excel", "application/x-excel",
                                 "application/vnd.ms-excel"),
            "xll"       => array("application/excel", "application/x-excel",
                                 "application/vnd.ms-excel"),
            "xlm"       => array("application/excel", "application/x-excel",
                                 "application/vnd.ms-excel"),
            "xlw"       => array("application/excel", "application/x-excel",
                                 "application/vnd.ms-excel",
                                 "application/x-msexcel"),
            "cod"       => "application/vnd.rim.cod",
            "arj"       => array("application/x-arj-compressed",
                                 "application/arj", "application/octet-stream"),
            ...
            "gz"        => array("application/x-gunzip", "application/x-gzip",
            ...
            "pl"        => array("application/x-perl", "text/plain",
                                 "text/x-script.perl"),
            "pm"        => array("application/x-perl", "image/x-xpixmap",
                                 "text/x-script.perl-module"),
            "pm4"       => "application/x-pagemaker",
            "pm5"       => "application/x-pagemaker",
            "rar"       => "application/x-rar-compressed",
            "sh"        => array("application/x-sh", "application/x-bsh",
                                 "application/x-shar", "text/x-script.sh"),
            ...
            "tgz"       => array("application/gnutar", "application/x-tar-gz",
                                 "application/x-compressed"),
            ...
            "snd"       => "audio/basic",
            "kar"       => "audio/midi",
            "mid"       => array("audio/midi", "application/x-midi",
                                 "audio/x-mid", "audio/x-midi",
                                 "music/crescendo", "x-music/x-midi"),
            "midi"      => array("audio/midi", "application/x-midi",
                                 "audio/x-mid", "audio/x-midi",
                                 "music/crescendo", "x-music/x-midi"),
            "mp2"       => array("audio/mpeg", "video/mpeg", "audio/x-mpeg",
                                 "video/x-mpeg", "video/x-mpeq2a"),
            "mp3"       => array("audio/mpeg", "audio/mpeg3", "audio/x-mpeg-3",
            ...
            "ra"        => array("audio/x-realaudio", "audio/x-pn-realaudio",
            ...
            "msh"       => "model/mesh",
            "silo"      => "model/mesh",
            "vrml"      => array("model/vrml", "x-world/x-vrml",
                                 "application/x-vrml"),
            "wrl"       => "model/vrml",
            ...
            "vivo"      => "video/vnd.vivo",
            "asf"       => "video/x-ms-asf",
            "asx"       => array("application/x-mplayer2", "video/x-ms-asx",
            ...
            "hlp"       => array("application/hlp", "application/x-helpfile",
            ...
            "list"      => "text/plain",
            "lst"       => "text/plain",
            "lzx"       => array("application/lzx", "application/octet-stream",
                                 "application/x-lzx"),
            ...
            "xla"       => array("application/excel", "application/x-excel",
                                 "application/x-msexcel"),
            "xlb"       => array("application/excel", "application/x-excel",
                                 "application/vnd.ms-excel"),
