 ...
 */
class Amasty_Customerattr_Adminhtml_AmCustomerAttrManageController
    extends Mage_Adminhtml_Controller_Action
{
        ...
        $this->_initAction()
            ->_addContent(
                $this->getLayout()->createBlock(
                    'amcustomerattr/adminhtml_customer_attribute'
                )
            )
            ->renderLayout();
    }
    ...
    protected function _initAction()
    {
        ...
        $this->loadLayout()
            ...
            ->_addBreadcrumb(
                Mage::helper('customer')->__('Customers'),
                Mage::helper('customer')->__('Customers')
            )
            ->_addBreadcrumb(
    ...
    public function editAction()
    {
        ...
        $this->_initAction()
            ->_addBreadcrumb(
                ...
                ), $id ? Mage::helper('catalog')->__('Edit Product Attribute')
                : Mage::helper('catalog')->__('New Product Attribute')
            )
            ->_addContent(
                $this->getLayout()->createBlock(
                    'amcustomerattr/adminhtml_customer_attribute_edit'
                )->setData(
                    'action', $this->getUrl('*/catalog_product_attribute/save')
                )
            )
            ->_addLeft(
                $this->getLayout()->createBlock(
                    'amcustomerattr/adminhtml_customer_attribute_edit_tabs'
                )
            )
            ->_addJs(
...
//            $model = Mage::getModel('catalog/entity_attribute');
            ...
            if ($id = $this->getRequest()->getParam('attribute_id')) {
                ...
                if ($model->getEntityTypeId() != $this->_entityTypeId) {
                    Mage::getSingleton('adminhtml/session')->addError(
                        Mage::helper('catalog')->__(
                            'You cannot update this attribute'
                        )
                    );
                    Mage::getSingleton('adminhtml/session')->setAttributeData(
                        $data
                    );
                    $this->_redirect('*/*/');
                    return;
            ...
            if (is_null($model->getIsUserDefined())
                ...
                $data['backend_type'] = $model->getBackendTypeByInput(
                    $data['frontend_input']
                );
            }
            ...
            $data['frontend_model']
                = "Amasty_Customerattr_Model_Rewrite_Eav_Entity_Attribute_Frontend_Default";
            $model->addData($data);
            ...
            try {
                $model->save();
                Mage::dispatchEvent('customer_attr_after_save');


                // saving Show on Manage Customers Grid, Show on Orders Grid and Show on Billing During Checkout
                $configuration = array(
                    'is_filterable_in_search' => Mage::app()->getRequest()
                        ->getPost('is_filterable_in_search'),
                    'used_in_product_listing' => Mage::app()->getRequest()
                        ->getPost('used_in_product_listing'),
                    'used_in_order_grid'      => Mage::app()->getRequest()
                        ->getPost('used_in_order_grid'),
                ...
                $model->getResource()->saveAttributeConfiguration(
                    $model->getId(), $configuration
                );
                ...
                $insertValue = $model->getDefaultValue();
                $isUnique = isset($data['is_unique']) && $data['is_unique'];
                //set all customers default value
                if (!$id && $insertValue && !$isUnique) {
                    $needDefaultValue = array(
                        "text",
                        "textarea",
                        "date",
                        "boolean",
                    ...
                    if (in_array($frontendInput, $needDefaultValue)) {
                        $attributeId = $model->getId();
                        $backendType = $model->getBackendType();
                        if ('date' == $frontendInput) {
                            $insertValue
                                = $fields = Mage::helper('amcustomerattr/guest')
                                ->formatDate($insertValue);
                        }
                        $resource = Mage::getSingleton('core/resource');
                        $connection = $resource->getConnection('core_read');
                        $table = $resource->getTableName('customer_entity');
                        $sql = "SELECT `entity_id` FROM $table";
                        $result = $connection->raw_query($sql);

                        $parts = array();
                        while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
                            $parts[]
                                = "(1," . $attributeId . "," . $row['entity_id']
                                . ",'" . $insertValue . "')";
                        }
                        if (!empty($parts)) {
                            $tableType = $resource->getTableName(
                                "customer_entity_$backendType"
                            );
                            $insertValues = implode(',', $parts);
                            $sql =
                                "INSERT INTO $tableType (`entity_type_id`,`attribute_id`,`entity_id`,`value`) VALUES "
                                . $insertValues;
                ...
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('amcustomerattr')->__(
                        'Customer attribute was successfully saved'
                    )
                );
                ...
                if ($this->getRequest()->getParam('popup')) {
                    $this->_redirect(
                        'adminhtml/catalog_product/addAttribute', array(
                    ...
                    $this->_redirect(
                        '*/*/edit', array('attribute_id' => $model->getId(),
                                          '_current'     => true)
                    );
                } else {
    ...
    public function deleteAction()
    {
        ...
        if ($model->getEntityTypeId() != $this->_entityTypeId) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('catalog')->__('You cannot delete this attribute')
            );
            $this->_redirect('*/*/');
            return;
        }
        ...
        try {
            ...
            Mage::dispatchEvent('customer_attr_after_delete');

            $relationDetails = Mage::getModel('amcustomerattr/details')
                ->usedInRelation($id);

            if ($relationDetails->getSize() <= 0) {
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('amcustomerattr')->__(
                        'Customer attribute was successfully deleted'
                    )
                );
                $this->_redirect('*/*/');
                return;
            ...
            Mage::getModel('amcustomerattr/details')->fastDelete(
                $deleteRelationDetails
            );
            $deleteRelationNames = array();
            foreach ($relationIds as $relationId) {
                if (!Mage::getModel('amcustomerattr/details')->haveDetails(
                    $relationId
                )
                ) {
            ...
            if ($deleteRelationNames) {
                Mage::getModel('amcustomerattr/relation')->fastDelete(
                    $deleteRelationNames
                );
            }
            Mage::getSingleton('adminhtml/session')->addSuccess(
                Mage::helper('amcustomerattr')->__(
                    'Customer attribute was successfully deleted (this attribute`s relations were successfully deleted, too)'
                )
            );
            ...
            $this->_redirect(
                '*/*/edit',
                array('attribute_id' => $this->getRequest()->getParam(
                    'attribute_id'
                ))
            );
            return;
