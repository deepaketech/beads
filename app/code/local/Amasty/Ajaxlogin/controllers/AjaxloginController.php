...
class Amasty_Ajaxlogin_AjaxloginController extends Mage_Customer_AccountController
{
    ...
    public function logoutAction()
    {
        ...
        $this->showCartPopup($this->__('Login or Create an Account'), $this->__('You are now logged out.'), "", 2);
    }
    
    //default magento login action
    public function loginAction()
    {
        $title = $this->__('Login or Create an Account');
        $text = "";
        $status = 3;
    ...
    public function _login($userInfo, $token, $type, $typeName)
    {
        ...
        if (Mage::getSingleton('customer/session')->isLoggedIn()) {
            if ($customerBySocialId) {
                $this->showCartPopup($this->__('Login or Create an Account'), $this->__('This account already have another %s user.', $typeName), "", 2);
                return;    
            }
        ...
        if ($customerByEmail) {
            ...
            $this->showCartPopup($this->__('Login or Create an Account'), $this->__('We have discovered you already have an account at our store. Your %s account is now connected to your store account.', $typeName), "", 2);
            return;
        }

        // Twitter response doesn't contain first_name and last_name, just name
        if ($type == 'tw' && isset($userInfo['name'])) {
            $position = strpos($userInfo['name'], ' ');
            if ($position > 0) {
                $userInfo['first_name'] = substr($userInfo['name'], 0, $position);
                $userInfo['last_name'] = substr($userInfo['name'], $position + 1);
            } else {
        ...
        if (empty($userInfo['first_name'])) {
            $this->showCartPopup($this->__('Login or Create an Account'), $this->__('Sorry, could not retrieve your %s first name. Please try again.', $typeName), "", 1);
            return;
        }

        if (empty($userInfo['last_name'])) {
            $this->showCartPopup($this->__('Login or Create an Account'), $this->__('Sorry, could not retrieve your %s last name. Please try again.', $typeName), "", 1);
            return;
        }
        
        if (empty($userInfo['email'])) {
            $this->showCartPopup($this->__('Login or Create an Account'), $this->__('Sorry, could not retrieve your %s email. Please try again.', $typeName), "", 1);
            return;
        }
        ...
        $this->showCartPopup($this->__('Login or Create an Account'), $this->__('Your %s account is now connected to your new user account at our store. Now you can login using our %s Connect button or using store account credentials you will receive to your email address.', $typeName, $typeName), "", 2);
    }
}
