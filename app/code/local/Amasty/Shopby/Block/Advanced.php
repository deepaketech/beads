...
class Amasty_Shopby_Block_Advanced extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawOpenCategoryItem($category, $level = 0)
    {
        ...
        $html = array();
		if($category->getId()!='2'){
			if($level=='1'){ $widthset='width:210px;'; }
			else if($level=='2'){ $widthset='width:180px;'; }
			else { $widthset='width:150px;'; }
			if($category->getProductCount()>=1){
				$esit = 'href='.$this->getCategoryUrl($category); 
			} else {
				$esit =  ' id="plusl stess" href="javascript:void(0)"  class="'.$category->getId().' cathover" style='.$widthset;
			}
			
        $html[1] = '<a '.$esit.'>' . $this->htmlEscape($category->getName()) . $productCount . '</a>';
		}
        ...
        if ($inPath || $showAsAll) {
            ...
            if ($children && count($children) > 0) {
                ...
                if($htmlChildren != '') {
                    $cssClass[] = 'expanded';
		if($category->getId()!='2'){ if($category->hasChildren()){ 
$display = 'style="display:none;"';
$plus = '<a id="plusl" href="javascript:void(0)" class="'.$category->getId().'"><i class="icon-plus-squared"></i></a>'; } }

$html[2] = $plus.'<ol id='.$category->getId().' '. $display.'>' . $htmlChildren . '</ol>

<script type="text/javascript">
jQuery( document ).ready(function() {   
jQuery(".'.$category->getId().'").click(function(){ 
        jQuery("#'.$category->getId().'").slideToggle();
		jQuery(".'.$category->getId().' i").toggleClass("icon-plus-squared icon-minus-squared");
		});
  });
 </script>';
                }
