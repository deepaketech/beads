 ...
 */
class Amasty_Stockstatus_Block_Rewrite_Product_View_Type_Configurable
    extends Amasty_Stockstatus_Block_Rewrite_Product_View_Type_Configurable_Pure
{
    protected $_options;
    ...
    protected function _afterToHtml($html)
    {
        ...
        if ('product.info.options.configurable' == $this->getNameInLayout() && !Mage::app()->getRequest()->isAjax()) {
            ...
            foreach ($allProducts as $product) {
                ...
                if ($key) {
                    ...
                    if ('true' == (string)Mage::getConfig()->getNode('modules/Amasty_Xnotif/active')) {
                        $stockAlert = Mage::helper('amxnotif')->getStockAlert(
                            $product,
            ...
            $tmpHtml .= 'var amStAutoSelectAttribute = '
                . (int)(Mage::getStoreConfig('amstockstatus/general/auto_select_attribute')) . '; ';

            $html = $tmpHtml
                . ' var stStatus = new StockStatus(' . Zend_Json::encode($aStockStatus) . ');</script>'
                . $html;
        }
        ...
        if (!$this->hasAllowProducts()) {
            ...
            foreach ($allProducts as $product) {
                ...
                if ($product->isSaleable()
                    || (Mage::getStoreConfig('amstockstatus/general/outofstock')
                        && !('true' == (string)Mage::getConfig()->getNode('modules/Amasty_Conf/active')
                            && Mage::registry('isList'))
                    )
                ) {
