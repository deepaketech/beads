...
class Amasty_Mostviewed_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    protected function _addCommonFilters($collection, $block)
    {
        ...
        if ($inStock) {
            ...
            // BEGIN: modified by Dzmitry Smolik (FKX-580-34951 - 2018-05-29)
            $qty = $this->getBlockConfig($block, 'qty');
            if ('' !== $qty) {
                $collection->getSelect()->where('at_inventory_in_stock.qty > ?', $qty);
            }
            // END: modified by Dzmitry Smolik (FKX-580-34951 - 2018-05-29)
        }
