    ...
    public function drawMenuItem($children, $level = 1, $type, $width, $mode = 'dt')
    {
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                ...
                $cid = $this->escapeHtml($child->getId());
                ...
                if(!$hide_item){
					if ($level == 1) {
						...
						if($i>4){ 
							...
							$pcat = Mage::getModel('catalog/category')->load($id);
							$pname = Mage::getModel('catalog/category')->load($pcat->getParentId());
							
							$html .= '<li class="'.$i . $class.' '.$active.'">';
							$html .= '<a class="level' . $level . '" href="' . $pname->getUrl() . '"><span><b>More '.$pname->getName().'</b></span></a>';
						}else {
						$html .= '<li class="'.$i . $class.' '.$active.'">';
						$html .= '<a class="level' . $level . '" href="' . $this->getCategoryUrl($child) . '"><span>' . $name .$label. '</span></a>';
						}
