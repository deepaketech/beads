...
class Mirasvit_SearchIndex_Model_Index_Mage_Cms_Block_Indexer extends Mirasvit_SearchIndex_Model_Indexer_Abstract
{
    protected function _getSearchableEntities($storeId, $entityIds, $lastEntityId, $limit = 100)
    {
        ...
        if ($entityIds) {
            $collection->addFieldToFilter('block_id', array('in' => $entityIds));
        }

        $collection->getSelect()->where('main_table.block_id > ?', $lastEntityId)
            ->limit($limit)
