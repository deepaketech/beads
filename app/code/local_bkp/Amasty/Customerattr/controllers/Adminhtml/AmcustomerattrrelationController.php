 ...
 */
class Amasty_Customerattr_Adminhtml_AmCustomerAttrRelationController
    extends Mage_Adminhtml_Controller_Action
{
        ...
        $this->_initAction()
            ->_addContent(
                $this->getLayout()->createBlock(
                    'amcustomerattr/adminhtml_customer_relation'
                )
            )
            ->renderLayout();
    }
    ...
    public function optionsAction()
    {
        ...
        foreach ($attributes as $attribute) {
            ...
            $depHtml .= '<option value="' . $attribute['value'] . '">'
    ...
    public function editAction()
    {
        ...
        if (count($attributes) == 0) {
            ...
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('amcustomerattr')->__(
                    'There are no attributes to create relations. Please create at least one attribute of the following type: Multiple Select; Multiple Checkbox Select with Images; Dropdown; Single Radio Select with Images; Customer Group Selector. Which will be used as a parent attribute.'
                )
            );
        ...
        $details = Mage::getModel('amcustomerattr/details')
            ->getResourceCollection()->getByRelation($model->getId());
        Mage::register('entity_relation', $model);
        Mage::register('entity_relation_details', $details);

        $this->_initAction()
            ->_addBreadcrumb(
                $id ? Mage::helper('amcustomerattr')->__('Edit Relation')
                    : Mage::helper('amcustomerattr')->__('New Relation'),
                $id ? Mage::helper('catalog')->__('Edit Relation')
                    : Mage::helper('catalog')->__('New Relation')
            )
            ->_addContent(
                $this->getLayout()->createBlock(
                    'amcustomerattr/adminhtml_customer_relation_edit'
                )
            )
            ->_addLeft(
                $this->getLayout()->createBlock(
                    'amcustomerattr/adminhtml_customer_relation_edit_tabs'
                )
            )
            ->renderLayout();
    }
    ...
    public function saveAction()
    {
        if ($data = $this->getRequest()->getPost()) {
            ...
            $detailsModel = Mage::getModel('amcustomerattr/details')
                ->getResource()->saveDetails($details);
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($relationIds)) {
            Mage::getSingleton('adminhtml/session')->addError(
                Mage::helper('amcustomerattr')->__('Please select item(s)')
            );
        } else {
            try {
                foreach ($relationIds as $relationId) {
                    $menu = Mage::getModel('amcustomerattr/relation')->load(
                        $relationId
                    );
                    $menu->delete();
                }
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('adminhtml')->__(
                        'Total of %d relation(s) were successfully deleted',
                        count($relationIds)
                    )
                );
            } catch (Exception $e) {
